...
allocate:
 ...
 # Initialize the memory manager if not done
 cmpl $0, heap_begin
 jne initialized
 call allocate_init
...
allocate_here:           #if we’ve made it here,
 ...
 # Is the request size smaller than
 # current region?
 # If yes, split it.
 cmpl %edx, %ecx
 jge mark_unavailable

 # Shrink the current region
 movl %ecx, HDR_SIZE_OFFSET(%eax)

 # Create a new region in the free space
 ...
 # Create a new header for the new region
 subl %ecx, %edx
